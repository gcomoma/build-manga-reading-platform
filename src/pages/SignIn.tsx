...
export default function SignIn() {
  ...
  const onSubmit = async (data: SignInFormData) => {
    ...
    try {
      ...
      console.log('Sign in attempt:', { email: data.email, rememberMe: data.rememberMe });
      ...
      if (data.email === 'demo@example.com' && data.password === 'password123') {
        // Success - store auth state and navigate
        ...
        <div className="bg-card/50 backdrop-blur-sm border border-border rounded-2xl p-8 shadow-lg">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-12 bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 text-primary-foreground font-medium transition-all duration-200 group disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button
              ...
              disabled={isLoading}
              className="h-11 border-border hover:bg-accent/50 transition-colors disabled:opacity-50"
            >
